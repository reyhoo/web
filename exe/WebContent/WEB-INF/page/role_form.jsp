...
<title>${isUpdate?"更新角色":"添加角色" }</title>
</head>
<body>
	...
	<fieldset>
		...
		<form action="${url }" method="post">
			id:<input type="text" name="id" readonly="readonly"
				value="${requestScope.role.id }"><br>
			<br> <input type="hidden" name="userId"
				value="${requestScope.role.userId }"> role_name:<input
				type="text" name="roleName" value="${requestScope.role.roleName }"><br>
			<br> note:<input type="text" name="note"
				value="${requestScope.role.note }"><br>
			<br> sex:
			<c:choose>
				<c:when test="${requestScope.role.sex.id == 0}">
			...
			<input type="hidden" name="pic" id="pic"
				value="${requestScope.role.pic }"> <span id="uploadField">文件：<input
				type="file" name="file" id="file1" style="display: inline;"
				value="上传文件"> <input type="button" id="uploadBtn" value="上传"></input></span><br>
			<br> <a href="#" id=imgLink><img id="img1" alt=""></a><br>
			<br> <input type="submit" value='${isUpdate ?"更新":"添加"}'>
		</form>
